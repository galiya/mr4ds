...
est_lm_nhood <- function(nhood = "Upper East Side") {
  ...
  lm_model <- lm(tip_amount ~ trip_distance, 
              data = filter(taxi_df, pickup_nhood == nhood))
...
make_formula <- function(depvar = dep_var, indepvars = indep_vars) {
...
```{r}
summary(est_lm_nhood("Upper West Side", make_formula()))
```
...
sapply(c("tip_amount", "fare_amount"), summarise_col) # simplifed output of "apply"
...
models <- lapply(c("Midtown", "East Village"), est_lm_nhood, model_form = make_formula())
```
